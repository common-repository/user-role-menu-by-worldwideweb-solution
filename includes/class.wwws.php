	...
	$('.tabbed-options')
		.find('a.rh-tab')
			.live('click',function(e){
				$(this).parents('.tabbed-options-tabs')
					.find('li')
				...
				$(this).parents('.tabbed-options')
					.find('.tabbed-option-tab-content')
						.hide()
						.end()
				;
...
//	add_filter( 'manage_nav-menus_columns', 'wp_nav_menu_manage_columns');
	...
	function derived_locations(){
		...
		if(is_array($locations)&&count($locations)>0){
			...
			foreach($locations as $location_slug => $location_label){
				...
				$derived_locations[$location_slug.'_logged_default'] = sprintf($label_format,$location_label,__('default logged in','wwws'));
	...
	function wp_nav_menu_locations_meta_box() {
		...
		$tab_labels = array(
			'public'=> __('Public','wwws'),
			'logged'=> __('Logged in','wwws'),
			'byrole'=> __('By User Role','wwws')
		);	
		...
		if( is_array($this->menu_tree) && count($this->menu_tree)>0 ){
			foreach($this->menu_tree as $slug => $sets){
				...
				foreach($sets as $tab_slug => $set_locations){
					...
					$tabs[]='<li><a class="rh-tab" href="#'.$tab_id.'">'.$tab_labels[$tab_slug].'</a></li>';
					$content = '';
